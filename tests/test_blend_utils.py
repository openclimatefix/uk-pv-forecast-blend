...
def test_convert_df_to_list_forecast_values():
    data = pd.DataFrame(
        columns=["target_time", "expected_power_generation_megawatts", "adjust_mw", "properties"],
        data=[["2020-01-01 00:00:00", 1, 2, {}], ["2020-01-01 00:00:00", 1, 2, {}]],
    )
...
def test_convert_df_to_list_forecast_values_whit_nan():
    data = pd.DataFrame(
        columns=["target_time", "expected_power_generation_megawatts", "adjust_mw", "properties"],
        data=[["2020-01-01 00:00:00", 1, 2, {}], ["2020-01-01 00:00:00", np.nan, 2, {}]],
    )

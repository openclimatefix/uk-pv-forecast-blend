...
import numpy as np
import pandas as pd
from nowcasting_datamodel.models import ForecastValue, InputDataLastUpdatedSQL
...
def blend_together_one_target_time(
    ...
    :param variable_to_blend: the variable to blend together
    ...
    expected_power_generation_megawatts = (
        np.sum(
            forecast_values_one_target_time[variable_to_blend]
    ...
    forecast_values_one_target_time_blend = pd.DataFrame(
        ...
        columns=[variable_to_blend, "adjust_mw", "weight"],
    )
...
def format_metadata(input_data: InputDataLastUpdatedSQL) -> Struct:
    """
    Format the metadata for the forecast.

    :param input_data: The input data to format
